...
Do note that all resume books begin with an empty string set for display type. Display type of a resume book lets it know
what item list to show.
...
We designed it this way because it no longer makes sense to redo the `add i/ ski ...` command. This is the behavior that most modern desktop applications follow.
