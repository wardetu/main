...
// tag::generateresume[]
=== [Proposed] Generate Resume feature
==== Proposed Implementation
The Generate Resume feature or `rgen` allows users to export a specific `Resume` item as a .pdf file, using the external
*iText* library. When using this feature, users must provide the index of the `Resume` item to be exported, as well as
the desired name of the generated file. The exported file will be saved in the root directory of the project.

`rgen` implements the following operations:

* `create(Template)` and `create()` - initialises the .pdf file, either using the input `Template` specified by user or
using the default `Template` if not specified.
* `addSection(String)` - starts a new section (e.g. "Internship").
* `populateSection(UniqueItemList)` and `insertItem(Item)` - populates the section with items in the same order as they
are in `UniqueItemList`. When an `Item` is inserted, it is correctly formatted depending on the item type.
...
Step 2. The user executes `rgen INDEX n\FILE_NAME [TEMPLATE_NAME]`. If any of the compulsory fields are missing, or the
the specified resume index is out of bound, ResuMé throws an error message.
...
The following screenshot illustrates what happens if you want to remove all the items in the resume, starting from a an already-filled resume:

image::ReditRemoveEverything.png[]

And the following screenshot illustrates what happens if you want to remove some items:

image::ReditWantSomeThings.png[]
