...
Step 1. The user launches the application, and uses the `add` command to add several resumes, internships, projects, and skills.

Step 2. The user executes `redit 1 int/ 2` command to add the second internship in the list of internships to the first resume in the list of resumes.

Step 3. This would call `ResumeBookParser#parseCommand()`, which would create a new `ResumeEditCommandParser` object and call the `ResumeEditCommandParser#parse()` method.

Step 4. A new `ResumeEditCommand` object is created. It contains the index of the `Resume` that is to be modified, and three `Optional<List<Integer>>` representing the indices of internships, projects and skills to be modified into the resume.
