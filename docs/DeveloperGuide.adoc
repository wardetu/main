...
the generated file, which will be saved in the <<root-directory, root directory>> of the project.
...
Step 2. The user executes `rgen INDEX n/ FILE_NAME`.

Step 3. If the specified resume `INDEX` is out of bound, *ResuMe* throws an error message. Otherwise, the application retrieves the correct `Resume` item, create a new `.pdf` file and populates it with the corresponding
