...
[NOTE]
--
* The default sort order is ascending. You can specify `reverse/` to reverse this ordering.
...
. Sort all resumes by their name in ascending order with `sort i/ res order/ name`.
+

image::
// TODO: add screenshot

. Sort all skills in descending level of proficiency with `sort i/ ski order/ level reverse/ true` .
+

image::
// TODO: add screenshot

. Sort all notes by their deadline in ascending order with `sort i/ note order/ time reverse/ false`, specifying
...
+
image::ReditListRes3.png[List,442,337]
...
+
image::ReditKeepSomeThings.png[KeepSomeThings,442,337]
